...
const NovelEditView = ({ novel, chapters, linkMode = 'detail', currentChapterId, isDark = false, viewCentered = false, setViewCentered }) => {
  ...
  const calculateNodePositions = (nodeList, connectionList) => {
    ...
    levelPositions.set(0, [rootNode.y]);
    ...
    while (queue.length > 0) {
      ...
      if (children.length > 0) {
        ...
        if (!levelPositions.has(nextLevel)) {
          levelPositions.set(nextLevel, []);
        }
        ...
        children.forEach((child, index) => {
          if (!visited.has(child.id)) {
            ...
            // 计算子节点的理想位置
            const idealY = node.y + (index - (children.length - 1) / 2) * 200;

            // 检查是否与同层其他节点重叠，如果重叠则调整位置
            let finalY = idealY;
            const usedPositions = levelPositions.get(nextLevel);

            // 检查是否与已使用的位置重叠，确保节点之间有足够空间
            while (usedPositions.some(pos => Math.abs(pos - finalY) < 150)) {
              finalY += 10; // 增加垂直间距，确保节点不重叠
            }

            child.y = finalY;
            usedPositions.push(finalY);
            visited.add(child.id);
            queue.push({ node: child, level: nextLevel });
          }
        });
      }
    }
  ...
  const drawNodes = (ctx) => {
    nodes.forEach(node => {
      ...
      // 节点背景
      const colors = getNodeColors(node.type, node.chapterType, node.id);
      ...
      // 计算内容总高度，用于居中布局
      const hasAuthor = novel.novel_type !== 'traditional' && node.authorUsername;
      const titleHeight = 15; // 标题高度
      const authorHeight = hasAuthor ? 12 : 0; // 作者高度
      const lineHeight = 12; // 正文行高
      ...
      for (let i = 0; i < previewText.length; i++) {
        const testLine = currentLine + previewText[i];
        const testWidth = ctx.measureText(testLine).width;
        if (testWidth > node.width - 16 && currentLine.length > 0) {
          lines.push(currentLine);
          currentLine = previewText[i];
        } else {
      ...
      const contentLines = Math.min(lines.length, 2); // 最多2行
      const contentHeight = contentLines * lineHeight;

      // 计算总内容高度
      const totalContentHeight = titleHeight + authorHeight + contentHeight;
      // 计算起始Y位置，使内容在节点中垂直居中
      ...
      ctx.fillText(title, node.x + node.width / 2, startY);

      // 绘制作者（只在非传统小说类型时显示）
      if (hasAuthor) {
        ctx.font = '9px system-ui';
        ctx.fillStyle = 'rgba(255, 255, 255, 0.8)';
        const authorText = `@${node.authorUsername}`;
        const authorWidth = ctx.measureText(authorText).width;
        if (authorWidth < node.width - 16) {
          ctx.fillText(authorText, node.x + node.width / 2, startY + titleHeight);
        }
      }
      ...
      const contentStartY = startY + titleHeight + authorHeight;
      lines.slice(0, 2).forEach((line, index) => {
        ctx.fillText(line, node.x + node.width / 2, contentStartY + index * lineHeight);
      });
  ...
  // 获取节点颜色
  const getNodeColors = (type, chapterType, nodeId) => {
    // 当前章节特殊高亮（仅在阅读模式下）
    if (linkMode === 'read' && currentChapterId && nodeId === currentChapterId) {
  ...
  const handlePointerUp = (e) => {
    ...
    const isClick = timeDiff < 300 && moveDistance < 5;
    ...
    if (isClick && draggedNode) {
      ...
      setSelectedChapter({
        ...
        chapterType: draggedNode.chapterType,
        authorId: draggedNode.authorId,
        authorUsername: draggedNode.authorUsername
      });
    }
  ...
  const resetView = () => {
    ...
    if (chapters && chapters.length > 0) {
      ...
      // 在阅读模式下，如果有当前章节，重置视图到当前章节居中位置
      if (linkMode === 'read' && currentChapterId) {
        const currentNode = positionedNodes.find(node => node.id === currentChapterId);
        const rootNode = positionedNodes.find(node => node.type === 'root');

        if (currentNode && rootNode) {
          // 计算当前章节相对于根节点的位置偏移
          const offsetX = rootNode.x - currentNode.x;
          const offsetY = rootNode.y - currentNode.y;
    ...
    // 在阅读模式下，重置视图居中状态
    if (linkMode === 'read' && setViewCentered) {
  ...
  const handlePan = (dx, dy) => {
    setCanvasState(prev => ({
      ...
      // 阅读模式下自动居中当前章节（只在需要时）
    useEffect(() => {
        if (linkMode === 'read' && currentChapterId && nodes.length > 0 && !viewCentered && setViewCentered) {
            const currentNode = nodes.find(node => node.id === currentChapterId);
            const rootNode = nodes.find(node => node.type === 'root');

            if (currentNode && rootNode) {
                // 计算当前章节相对于根节点的位置偏移
                const offsetX = (rootNode.x - currentNode.x) * canvasState.scale;
                const offsetY = (rootNode.y - currentNode.y) * canvasState.scale;
    ...
    }, [nodes, currentChapterId, linkMode, canvasState.scale, viewCentered, setViewCentered]);
  ...
  return (
    <div className="h-full flex flex-col min-h-0">
      ...
      <div className="flex-1 relative min-h-0">
        ...
        {/* 右侧信息面板 - 阅读模式使用侧边栏效果，编辑模式PC端悬浮在画布上 */}
        {selectedChapter && (
          <div className={`fixed inset-y-0 right-0 z-50 w-80 border-l shadow-xl transform transition-all duration-300 ease-in-out ${linkMode !== 'read' ? 'lg:absolute lg:inset-auto lg:z-auto lg:w-80 lg:max-w-none lg:shadow-none lg:border lg:rounded-lg lg:top-4 lg:right-4 lg:bottom-4' : ''} ${isDark ? '' : 'bg-white'} ${isPanelOpen
